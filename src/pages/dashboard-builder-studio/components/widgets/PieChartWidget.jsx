...
const PieChartWidget = ({ widget, data, loading, error, onDrill, filters }) => {
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-muted/30 rounded border border-dashed border-border">
        <div className="text-center">
          <Icon name="PieChart" size={32} className="mx-auto mb-2 text-muted-foreground" />
    ...
    <div className="w-full h-full">
      <ResponsiveContainer width="100%" height="100%">
        <RechartsPieChart>
          <Pie
            data={chartData}
            dataKey={widget.valueField || 'value'}
            nameKey={widget.labelField || 'name'}
            cx="50%"
            cy="50%"
            outerRadius={100}
